		...
		if(this.open) {
			if(this.lock && this.voters.get(user.id)) {
				...
				return {
					success: true,
					reason: '',
					message: `Great, I logged your vote for "${this.choices.get(key).name}"${user.username}!`
				};
			}
		} else {
			return {
...
client.on('message', message => {
    if(message.channel.id == 559088290381168651||548617194821648406){
	if(message.content) {
